...
public class LockCommand implements BuildSystemCommandExecutor {

    private BuildSystem plugin;

    /**
     * "This function is called when the plugin is enabled, and it sets the command executor for the command 'lock' to this
     * class."
     *
    ...
    /**
     * If the player has permission to lock worlds, then lock the world they're in
     *
        ...
        if (cs instanceof Player) {
            ...
            for (WorldData wd : WorldData.getWORLDS()) {
                if (wd.getName().equalsIgnoreCase(world)) {
                    if (wd.isLocked()) {
                        for (String s : lang.getStringList("MESSAGES.ALREADY-LOCKED")) {
                            player.sendMessage(ColorUtils.colorize(s));
                        }
                        break;
                    }
                    wd.toggleLock();
                    wd.save(plugin);
                    for (String s : lang.getStringList("MESSAGES.WORLD-LOCKED")) {
